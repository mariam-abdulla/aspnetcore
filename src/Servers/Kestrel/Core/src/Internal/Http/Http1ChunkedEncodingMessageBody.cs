...
internal sealed class Http1ChunkedEncodingMessageBody : Http1MessageBody
{
    ...
    private void ParseExtensionIncludingCRLF(ReadOnlySequence<byte> buffer, out SequencePosition consumed, out SequencePosition examined)
    {
        ...
        while (true)
        {
            ...
            // After the extension name (and optionally whitespace), we didn't find a semicolon.
            // We will now advance over the last byte.
            // This is the byte stored in "afterExtensionNameAndBWS" which we previously only peeked into.
            reader.Advance(1);
            examined = reader.Position;
